...
if df_variants_frequency is not None and df_intersect_with_dbsnp is not None and df_transcript_info is not None:
    ...
    descriptions = {
        ...
        "splice_start_position": "The start position of the splice site.",
        "splice_end_position": "The end position of the splice site.",
        ...
        "splice_sites_affected": "The type of splice site affected (acceptor or donor).",
        ...
        "Loss or Gain of Function based on LogOddRatio": "The score generated by DNABERT. A positive score indicates splice disruption, while a negative score indicates gain of function."
    }
    # Smalldescribtion
    ...
    with col2:
        try:
            ...
            unique_variants = df_variants_frequency[['chromosome', 'variant_start_position', 'variant_end_position']].drop_duplicates()
            variant_count = unique_variants.shape[0]
            unique_splice_sites = df_variants_frequency[['chromosome', 'splice_start_position', 'splice_end_position']].drop_duplicates()
            splice_site_count = unique_splice_sites.shape[0]
            acceptor_count = df_variants_frequency[df_variants_frequency['splice_sites_affected'] == 'acceptor'][['chromosome', 'splice_start_position', 'splice_end_position']].drop_duplicates().shape[0]
            donor_count = df_variants_frequency[df_variants_frequency['splice_sites_affected'] == 'donor'][['chromosome', 'splice_start_position', 'splice_end_position']].drop_duplicates().shape[0]

            # Create a DataFrame to display these counts
            data = {
                "Description": ["Number of Unique Variant Regions", "Number of Unique Splice Site Regions", "Number of Unique Acceptor Splice Sites", "Number of Unique Donor Splice Sites"],
                "Count": [variant_count, splice_site_count, acceptor_count, donor_count]
            }
            count_df = pd.DataFrame(data)
            st.dataframe(count_df)  # Display DataFrame
            st.markdown("<div>Data Statistics of the Predicted Candidate {}.</div>".format(analysis_type.lower()), unsafe_allow_html=True)
